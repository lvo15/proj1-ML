    ...
    def trainDT(self, X, Y, maxDepth, used):
        ...
        if maxDepth <= 0 or len(util.uniq(Y)) <= 1:
            ...
            for d in range(D):
                ...
                leftError = len(leftY) - leftY.tolist().count(util.mode(leftY)) if len(leftY) > 0 else 0
                rightError = len(rightY) - rightY.tolist().count(util.mode(rightY)) if len(rightY) > 0 else 0
            ...
            if bestFeature < 0:
                ...
                leftMask = X[:, bestFeature] < 0.5
                rightMask = X[:, bestFeature] >= 0.5
                leftX = X[leftMask]
                leftY = Y[leftMask]
                rightX = X[rightMask]
                rightY = Y[rightMask]
                ...
                self.left.trainDT(leftX, leftY, maxDepth-1, used + [bestFeature])
                self.right.trainDT(rightX, rightY, maxDepth-1, used + [bestFeature])
