...
class AlwaysPredictMostFrequent(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        ### TODO: YOUR CODE HERE
        counts = Counter(Y)
        self.mostFrequentClass = counts.most_common(1)[0][0]
...
class FirstFeatureClassifier(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        ### TODO: YOUR CODE HERE
        pos_labels = [y for (x, y) in zip(X, Y) if x[0] > 0]
        neg_labels = [y for (x, y) in zip(X, Y) if x[0] <= 0]

        if pos_labels:
            self.classForPos = Counter(pos_labels).most_common(1)[0][0]
        if neg_labels:
            self.classForNeg = Counter(neg_labels).most_common(1)[0][0]
